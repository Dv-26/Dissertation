...
\section{JPEG编码系统硬件结构设计}
  ...
  \subsection{DCT模块}
    ...
    \subsection{Zigzag扫描模块}
      ...
      Zigzag扫描模块的内部结构划分如图\ref{zigzag module}所示，当工作时该模块
      输出Zigzag扫描每个时刻扫描的8×8单元的坐标值$(x,y)$。并将两个坐标值进行高低
      位拼接作为RAM的读地址。该模块各个输入输出信号的功能如表\ref{zigzag io}所示。
      \begin{figure}[H]
        \centering
      ...
      \subsubsection{zigzao扫描算法}
        \vspace{1em}
        由图\ref{zigzag module}所示。该模块使用一个状态来控制两个代表坐标值的计数器
        ...
        同时表示该信号此时有效。与流程图类似棱形框代表状态输入信号的判断。圆角矩形
        内部存放米利型输出。下面分别用三个状态描述zigzag扫描的运行步骤。
